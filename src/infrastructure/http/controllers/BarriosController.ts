...
export class BarriosController {
   ...
   */
  async buscarPorCoordenadas(req: Request, res: Response): Promise<void> {
    try {
      const longRaw = req.query.longitud as string;
      const latRaw = req.query.latitud as string;
   ...
   */
  async obtenerGeometria(req: Request, res: Response): Promise<void> {
    try {
      const id = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
