...
import { CrearDistritoMunicipalDto, ActualizarDistritoMunicipalDto } from '../../../application/dtos/DistritoMunicipalDtos';
import { DistritoMunicipalYaExisteError } from '../../../domain/errors/DistritosMunicipales/DistritoMunicipalYaExisteError';
import { VerificarValor } from '../../../domain/errors/Estados/VerificarValor';

export class DistritosMunicipalesController {
  private gestionarDistritosUseCase: GestionarDistritosMunicipalesUseCase;

  constructor() {
    this.gestionarDistritosUseCase = container.resolve(GestionarDistritosMunicipalesUseCase);
  }
   ...
   */
  async buscarPorMunicipio(req: Request, res: Response): Promise<void> {
    try {
  ...
  async crear(req: Request, res: Response): Promise<void> {
    try {
      const dto: CrearDistritoMunicipalDto = req.body;
  ...
  async actualizar(req: Request, res: Response): Promise<void> {
    try {
      ...
      const dto: ActualizarDistritoMunicipalDto = {
        id,
        ...req.body
      };
  ...
  private manejarError(error: any, res: Response): void {
    if (error instanceof DistritoMunicipalYaExisteError) {
      res.status(409).json({
        success: false,
