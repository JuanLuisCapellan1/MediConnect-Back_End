...
export class ResenaController {
    constructor(
        @inject(GestionarResenasUseCase)
        private resenasUseCase: GestionarResenasUseCase
    ) { }
    ...
    async listarPorServicio(req: Request, res: Response): Promise<void> {
        try {
            const servicioId = Number(req.params.servicioId);
            if (isNaN(servicioId)) { res.status(400).json({ success: false, message: 'ID de servicio inválido.' }); return; }
    ...
    async listarPorDoctor(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (isNaN(doctorId)) { res.status(400).json({ success: false, message: 'ID de doctor inválido.' }); return; }
    ...
    async misResenas(req: Request, res: Response): Promise<void> {
        try {
            const pacienteId = req.user?.userId;
            if (!pacienteId) { res.status(401).json({ success: false, message: 'No autenticado.' }); return; }
    ...
    async eliminar(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (isNaN(resenaId)) { res.status(400).json({ success: false, message: 'ID de reseña inválido.' }); return; }
    ...
    private manejarError(error: any, res: Response): void {
        ...
        if (error?.code === 'P2002') {
            res.status(409).json({ success: false, message: 'Ya existe una reseña para este servicio.' }); return;
        }
