 ...
 */
export class NotificarMensajesPendientesService {
    private prisma: PrismaClient;
    private enviarNotifUC: EnviarNotificacionUseCase;

    // Parámetros
    private static readonly TIEMPO_RETRASO_MINUTOS = 20;
    private static readonly INTERVALO_CRON = '*/5 * * * *'; // Cada 5 minutos

    constructor(prisma: PrismaClient, enviarNotifUC: EnviarNotificacionUseCase) {
        this.prisma = prisma;
        this.enviarNotifUC = enviarNotifUC;
    }

    public iniciar(): void {
        cron.schedule(NotificarMensajesPendientesService.INTERVALO_CRON, async () => {
    ...
    private async _procesarMensajesPendientes(): Promise<void> {
        ...
        for (const lectura of lecturas) {
            ...
            if (msjNoLeidos.length === 0) continue;
            ...
            if (ultimoMensaje.remitenteId === lectura.usuarioId) continue;
            ...
            const tienePendientes =
                lectura.ultimoMensajeLeidoId === null ||
