...
exports.DistritoMunicipalValidator = void 0;
const DistritoMunicipalYaExisteError_1 = require("../../errors/DistritosMunicipales/DistritoMunicipalYaExisteError");
class DistritoMunicipalValidator {
    constructor(distritosRepository, municipiosRepository) {
     ...
     */
    async validarCreacion(nombre, municipioId) {
        ...
        const distritoExistente = distritosEnMunicipio.some(d => d.nombre.toLowerCase().trim() === nombre.toLowerCase().trim());
        if (distritoExistente) {
            throw new DistritoMunicipalYaExisteError_1.DistritoMunicipalYaExisteError(nombre, municipioId);
        }
    }
    /**
     ...
     */
    async validarActualizacionMunicipio(municipioId, distritoId) {
        ...
        const distritosEnMunicipio = await this.distritosRepository.listarPorMunicipio(municipioId);
        const distritoExistente = distritosEnMunicipio.some(d => d.nombre.toLowerCase().trim() === distrito.nombre.toLowerCase().trim() && d.id !== distritoId);
        if (distritoExistente) {
            throw new DistritoMunicipalYaExisteError_1.DistritoMunicipalYaExisteError(distrito.nombre, municipioId);
        }
    }
}
exports.DistritoMunicipalValidator = DistritoMunicipalValidator;
