...
exports.BarrioValidator = void 0;
const BarrioYaExisteError_1 = require("../../errors/Barrios/BarrioYaExisteError");
class BarrioValidator {
    constructor(barriosRepository, seccionesRepository) {
     ...
     */
    async validarCreacion(nombre, seccionId) {
        ...
        const barrioExistente = barriosEnSeccion.some(b => b.nombre.toLowerCase().trim() === nombre.toLowerCase().trim());
        if (barrioExistente) {
            throw new BarrioYaExisteError_1.BarrioYaExisteError(nombre, seccionId);
        }
    }
    /**
     ...
     */
    async validarActualizacionSeccion(seccionId, barrioId) {
        ...
        if (barrio.seccionId !== seccionId) {
            ...
            const barrioExistente = barriosEnNuevaSeccion.some(b => b.nombre.toLowerCase().trim() === barrio.nombre.toLowerCase().trim());
            if (barrioExistente) {
                throw new BarrioYaExisteError_1.BarrioYaExisteError(barrio.nombre, seccionId);
            }
        }
    }
}
exports.BarrioValidator = BarrioValidator;
