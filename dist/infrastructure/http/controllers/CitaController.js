...
let CitaController = class CitaController {
    ...
    async disponibilidadDoctor(req, res) {
        try {
    ...
    async slotsDisponiblesParaServicio(req, res) {
        try {
            const servicioId = Number(req.params.id);
    ...
    async historialPaciente(req, res) {
        try {
            const pacienteId = req.user?.userId;
            if (!pacienteId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
    ...
    async registrarInactividad(req, res) {
        try {
            const doctorId = req.user?.userId;
            if (!doctorId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
    ...
    async listarInactividades(req, res) {
        try {
            const doctorId = req.user?.userId;
            if (!doctorId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
    ...
    async cancelarInactividad(req, res) {
        try {
            const doctorId = req.user?.userId;
            if (!doctorId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
            ...
            if (isNaN(periodoId)) {
                res.status(400).json({ success: false, message: 'ID de periodo inválido.' });
                return;
            }
    ...
    async calendario(req, res) {
        try {
            ...
            const data = await this.citasUseCase.calendarioCitas(usuarioId, rol, { vista: vista, fecha });
            res.status(200).json({ success: true, ...data });
        }
        catch (error) {
            this.manejarError(error, res);
    ...
    async estadisticasPacientes(req, res) {
        try {
            const doctorId = req.user?.userId;
            if (!doctorId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
    ...
    async listarMisPacientes(req, res) {
        try {
            const doctorId = req.user?.userId;
            if (!doctorId) {
                res.status(401).json({ success: false, message: 'No autenticado' });
            ...
            const filtros = {
                ...
                ultimaCitaDesde: ultimaCitaDesde,
                ultimaCitaHasta: ultimaCitaHasta,
            };
...
exports.CitaController = CitaController = __decorate([
    ...
    __param(0, (0, tsyringe_1.inject)(GestionarCitasUseCase_1.GestionarCitasUseCase)),
    __metadata("design:paramtypes", [GestionarCitasUseCase_1.GestionarCitasUseCase])
], CitaController);
