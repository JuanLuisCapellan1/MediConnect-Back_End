...
class PrismaCitaRepository {
    ...
    async listarPacientesDelDoctor(doctorId, filtros) {
        ...
        if (filtros.tieneAlergias !== undefined) {
            if (filtros.tieneAlergias === true) {
                ...
                pacientes = pacientes.filter(p => p.condiciones.lista.some((c) => c.tipo === 'Alergia'));
            }
            else {
    ...
    async listarFuturasCitas(doctorId, pacienteId, desde) {
        const futurasCitas = await this.prisma.cita.findMany({
            where: {
                doctorUsuarioId: doctorId,
