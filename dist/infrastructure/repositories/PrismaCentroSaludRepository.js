...
class PrismaCentroSaludRepository {
    ...
    async buscarCercanos(lat, lng, radioKm, filtros, doctorId) {
        ...
        const condiciones = [];
        if (filtros?.tipoCentroId) {
            condiciones.push(client_1.Prisma.sql `cs.id_tipo_centro = ${filtros.tipoCentroId}`);
        }
        if (filtros?.nombre) {
            const termino = `%${filtros.nombre}%`;
            condiciones.push(client_1.Prisma.sql `LOWER(cs.nombre_comercial) ILIKE LOWER(${termino})`);
        }
      ...
      `;
        ...
        const ubicIds = centros.map((c) => c.ubicacion?.id).filter(Boolean);
        if (ubicIds.length > 0) {
            const geoRows = await this.prisma.$queryRaw `
        SELECT
          u.id_ubicacion                     AS id,
      ...
      `;
            ...
            for (const c of centros) {
                if (!c.ubicacion?.id)
                    continue;
                const geo = geoMap.get(c.ubicacion.id);
                if (!geo)
                    continue;
                c.ubicacion.latitud = geo.latitud != null ? Number(geo.latitud) : null;
                c.ubicacion.longitud = geo.longitud != null ? Number(geo.longitud) : null;
                const partes = [];
                if (c.ubicacion.direccion)
                    partes.push(c.ubicacion.direccion.trim());
                if (geo.barrio_nombre)
                    partes.push(geo.barrio_nombre.trim());
                if (geo.municipio_nombre)
                    partes.push(geo.municipio_nombre.trim());
    ...
    async estadisticasGenerales(centroSaludId) {
        ...
        const ratings = solicitudes
            .map((s) => s.doctor.calificacionPromedio)
            .filter((r) => r != null)
            .map((r) => parseFloat(r.toString()));
        const valoracionPromedio = ratings.length > 0
            ? Math.round((ratings.reduce((acc, r) => acc + r, 0) / ratings.length) * 100) / 100
            : null;
        return { totalMedicos, totalEspecialidades, citasSemanaActual, valoracionPromedio };
    }
    async crecimientoMedicos(centroSaludId, periodo) {
        const now = new Date();
        ...
        const fechas = alianzas.map((a) => new Date(a.actualizadoEn ?? a.creadoEn));
        ...
        if (groupBy === 'dia') {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                puntoMap.set(key, 0);
                puntoMap.__label = puntoMap.__label ?? {};
                (puntoMap.__label)[key] = DIAS_ES[d.getUTCDay()];
            }
        }
        else if (groupBy === 'semana') {
            for (let sem = 1; sem <= 4; sem++)
                puntoMap.set(`Sem ${sem}`, 0);
        }
        else {
        ...
        for (const fecha of fechas) {
            ...
            if (puntoMap.has(key))
                puntoMap.set(key, (puntoMap.get(key) ?? 0) + 1);
        }
    ...
    async distribucionEspecialidades(centroSaludId) {
        // Doctores afiliados
        const solicitudes = await this.prisma.solicitudAlianza.findMany({
            where: { centroSaludId, estado: 'Aceptada' },
            select: {
        ...
        for (const s of solicitudes) {
            for (const esp of s.doctor.especialidades) {
                ...
                if (!conteoMap.has(id))
                    conteoMap.set(id, { nombre, count: 0 });
