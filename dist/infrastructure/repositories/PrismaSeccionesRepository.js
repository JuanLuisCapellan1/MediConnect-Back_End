...
class PrismaSeccionesRepository {
    ...
    async obtenerPorMunicipio(municipioId, estado) {
        ...
        const secciones = await this.prisma.$queryRawUnsafe(`
      SELECT DISTINCT
        s.id_seccion,
    ...
    `, municipioId);
        const mapped = secciones.map((s) => new Seccion_1.Seccion(s.id_seccion, s.id_distrito_municipal, s.nombre, s.estado, s.creado_en));
        await this.redis.set(cacheKey, JSON.stringify(mapped), this.CACHE_TTL);
        return mapped;
    }
