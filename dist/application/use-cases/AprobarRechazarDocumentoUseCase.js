...
let AprobarRechazarDocumentoUseCase = class AprobarRechazarDocumentoUseCase {
    ...
    async execute(adminId, dto) {
        ...
        let documentoDoctor = null;
        let documentoCentro = null;
        if (accion.documentoId) {
            documentoDoctor = await client_1.prisma.documentoDoctor.findUnique({
                where: { id: accion.documentoId },
                select: { id: true, doctorId: true, tipoDocumento: true },
            });
            if (!documentoDoctor)
                throw new Error('Documento del doctor no encontrado');
        }
        else if (accion.id_documento_centro) {
            documentoCentro = await client_1.prisma.documentos_centros.findUnique({
            ...
            if (!documentoCentro)
                throw new Error('Documento del centro no encontrado');
        }
        ...
        await client_1.prisma.$transaction(async (tx) => {
            ...
            if (esRegistroCentro) {
                ...
                if (dto.decision === 'Aprobada') {
                    ...
                    const todosDocumentosAprobados = documentosCentro.length > 0 &&
                ...
                if (dto.decision === 'Aprobada') {
                    ...
                    const todosDocAprobados = todosDocumentos.length > 0 &&
                ...
                if (dto.decision === 'Aprobada') {
                    ...
                    const todosDocumentosAprobados = documentos.length > 0 &&
                ...
                if (dto.decision === 'Aprobada') {
                    ...
                    const todosDocumentosAprobados = documentos.every((doc) => doc.estadoRevision === 'Aprobado');
        ...
        try {
            if (esRegistroCentro) {
                ...
                const todosDocsAprobadosNotif = dto.decision === 'Aprobada'
                    ? await (async () => {
                        const docs = await client_1.prisma.documentos_centros.findMany({
                            where: { id_centro_salud: documentoCentro.id_centro_salud, estado: 'Activo' },
                            select: { estado_revision: true },
                        });
