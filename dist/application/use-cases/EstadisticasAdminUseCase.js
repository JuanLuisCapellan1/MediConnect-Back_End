...
exports.EstadisticasAdminUseCase = void 0;
...
function resolverRangoAnterior(periodo) {
    const ahora = new Date();
    switch (periodo) {
        case 'semana': {
...
function calcularCambioPorcentaje(actual, anterior) {
    if (anterior === 0)
        return actual > 0 ? 100 : 0;
    return Math.round(((actual - anterior) / anterior) * 100 * 10) / 10;
}
function agruparPorPeriodo(registros, periodo, campoFecha) {
    const ahora = new Date();
    if (periodo === 'semana') {
...
class EstadisticasAdminUseCase {
    ...
    async obtenerResumen(periodo = 'mes') {
        ...
        const [totalPacientes, totalDoctores, totalCentros] = await Promise.all([
            client_1.prisma.paciente.count(),
            client_1.prisma.doctor.count(),
            client_1.prisma.centroSalud.count(),
        ]);
        ...
        if (periodo !== 'todo') {
            [pacientesPeriodo, doctoresPeriodo, centrosPeriodo] = await Promise.all([
                client_1.prisma.paciente.count({ where: { creadoEn: { gte: rango.inicio, lte: rango.fin } } }),
                client_1.prisma.doctor.count({ where: { creadoEn: { gte: rango.inicio, lte: rango.fin } } }),
                client_1.prisma.centroSalud.count({ where: { creadoEn: { gte: rango.inicio, lte: rango.fin } } }),
            ]);
            if (rangoAnterior) {
                [pacientesAnterior, doctoresAnterior, centrosAnterior] = await Promise.all([
                    client_1.prisma.paciente.count({ where: { creadoEn: { gte: rangoAnterior.inicio, lte: rangoAnterior.fin } } }),
                    client_1.prisma.doctor.count({ where: { creadoEn: { gte: rangoAnterior.inicio, lte: rangoAnterior.fin } } }),
                    client_1.prisma.centroSalud.count({ where: { creadoEn: { gte: rangoAnterior.inicio, lte: rangoAnterior.fin } } }),
                ]);
            }
        }
        return {
     ...
     */
    async obtenerConsultas(periodo = 'año') {
        const rango = resolverRango(periodo);
        const citas = await client_1.prisma.cita.findMany({
            where: {
                fechaInicio: {
     ...
     */
    async obtenerUsuarios(periodo = 'año') {
        const rango = resolverRango(periodo);
        const usuarios = await client_1.prisma.usuario.findMany({
            where: {
                creadoEn: {
                    ...(periodo !== 'todo' ? { gte: rango.inicio } : {}),
                    lte: rango.fin,
     ...
     */
    async obtenerServiciosDistribucion(periodo = 'año', limite = 8) {
        const rango = resolverRango(periodo);
        const grupos = await client_1.prisma.cita.groupBy({
            by: ['servicioId'],
            where: {
                fechaInicio: {
                    ...(periodo !== 'todo' ? { gte: rango.inicio } : {}),
        ...
        const servicios = await client_1.prisma.servicio.findMany({
     ...
     */
    async obtenerTipoConsulta(periodo = 'año') {
        const rango = resolverRango(periodo);
        const grupos = await client_1.prisma.cita.groupBy({
            by: ['modalidad'],
            where: {
     ...
     */
    async obtenerTopEspecialidades(limite = 5) {
        // Obtener reseñas activas con calificación y doctor_id
        const resenas = await client_1.prisma.resena.findMany({
        ...
        const relaciones = await client_1.prisma.doctorEspecialidad.findMany({
            where: {
...
exports.EstadisticasAdminUseCase = EstadisticasAdminUseCase;
